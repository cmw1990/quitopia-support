...
const TOKEN_REFRESH_INTERVAL = 12 * 60 * 60 * 1e3;
...
const AuthProvider = ({
  ...
  const extendSessionExpiration = (sessionData) => {
    if (!sessionData)
      return null;
    ...
    const extendedSession = {
      ...sessionData,
      expires_at: Math.floor(expiresAt.getTime() / 1e3)
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data?.access_token) {
          const extendedSession = extendSessionExpiration(data);
  ...
  useEffect(() => {
    const checkSession = async () => {
      try {
        const storedSession = getSessionToken();
        if (storedSession?.access_token) {
          ...
          if (response.ok) {
            ...
            const extendedSession = extendSessionExpiration({
              ...storedSession,
              user: userData
            });
            setSession(extendedSession);
          } else if (storedSession.refresh_token) {
            const refreshed = await refreshToken(storedSession.refresh_token);
            if (!refreshed) {
    ...
    const refreshInterval = setInterval(() => {
      ...
      if (storedSession?.refresh_token) {
        refreshToken(storedSession.refresh_token).catch(console.error);
      }
  ...
  const signIn = async (email, password) => {
    try {
      ...
      if (data?.access_token) {
        const extendedSession = extendSessionExpiration(data);
        setSession(extendedSession);
        ue.success("Successfully signed in");
        navigate("/app");
      }
    } catch (error) {
      console.error("Error signing in:", error);
      ue.error(error instanceof Error ? error.message : "Failed to sign in");
      throw error;
    } finally {
      setIsLoading(false);
    }
  ...
  const signOut = async () => {
    try {
      ...
      localStorage.removeItem("supabase.auth.token");
      setSession(null);
      ue.success("Successfully signed out");
      navigate("/");
    } catch (error) {
      console.error("Error signing out:", error);
      ue.error("Failed to sign out");
      localStorage.removeItem("supabase.auth.token");
      setSession(null);
      navigate("/");
      throw error;
...
export { AuthProvider as A, ue as a, useAuth as u };
